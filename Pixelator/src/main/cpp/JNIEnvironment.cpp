...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  auto miniScreenClass = env->FindClass(MINISCREEN);
  auto result2 = env->RegisterNatives(miniScreenClass, miniscreenNativeMethods, sizeof(miniscreenNativeMethods) / sizeof(miniscreenNativeMethods[0]));

  env->DeleteLocalRef(miniScreenClass);
